...
import type { HasTrackModifications } from "../../types/guardingFunctions/trackModificationsGuards";
import {
  type ModificationsProp,
  isModificationProp,
} from "../../types/modifications.types/ModificationProps.type";
import type {
  TrackModifications,
...
const idBuilders: IdBuilders = {
  ...
  addClassChoice: (mod) => `${mod.name}: ${mod.source}-${mod.type}`,
  addCustomClassChoice: (mod) => `${mod.name}: ${mod.source}-${mod.type}`,
  addProficiencyToArmor: (mod) =>
    `${mod.name}: ${mod.source}-${mod.type}-${mod.armorType}`,
  addProficiencyToWeapon: (mod) =>
    `${mod.name}: ${mod.source}-${mod.type}-${mod.weaponProficiency}`,
  addExtraCantripKnownToClass: (mod) =>
    `${mod.name}: ${mod.source}-${mod.type}-${mod.quantity}`,
};
