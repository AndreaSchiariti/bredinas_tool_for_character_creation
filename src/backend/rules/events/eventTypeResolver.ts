...
import { removeFromTrackModificationsById } from "../calculationsAndBuilders/idBuilder";
import { isBlockedByInternalFeatureLimitation } from "../limitations/internalFeatureLimitation";
import {
  diceAndNumbersToString,
  getTarget,
} from "../modifications/modificationsExecution";
import type { CharacterAttacks } from "../../types/features.types/attacks.types";
import { isTargetInterface, isDiceInterface } from "../../types/guardingFunctions/generalFeaturesGuards";
import { hasValueProperty, hasDiceProperty } from "../../types/guardingFunctions/trackModificationsGuards";
import { hasReplenishMaxAndRemainingUses, isContinuousEventWithTriggerCounter, type CountersInterface } from "../../types/modifications.types/counters.types";
import { calculateModifyValue } from "../calculationsAndBuilders/characterCalculations";
import { isSameDice } from "../calculationsAndBuilders/diceCalculations";
import { modificationIsApplied, relatedModStillActive } from "../calculationsAndBuilders/modCommonFunctions/generalModFunctions";
import { addResistanceSet, activateConditionSet, addAdvantageSet } from "../calculationsAndBuilders/modCommonFunctions/modSets";
...
export interface EventTypeMap {
  ...
  addAdvantageEvent: {
    character: Character;
    target: CharacterAttacks[] | SkillProp[];
...
function onAddingResistances(
  ...
  return {
    result: target.map((damageType) => {
      ...
      if (!eventActive) {
        updatedTrackModification = removeFromTrackModificationsById(
          damageType,
          counterRef.id,
        );
      }
  ...
  return {
    result: {
      ...
      conditionsList: target.conditionsList.map((condition) => {
        ...
        const conditionStillActive = relatedModStillActive(
          updatedTrackModification,
...
function onAddingAdvantageEvent(
  ...
  target: CharacterAttacks[] | SkillProp[],
  event: Extract<EventCounterProp, { type: "addAdvantageEvent" }>,
): EventReturn<CharacterAttacks[] | SkillProp[]> {
  const counterRef = getTarget(character, event.activeCounterRef);
  ...
  if (event.scope === "skills") {
    ...
    const skill = target as SkillProp[];

    return {
      result: skill.map((feature) => {
