...
import type { AllTargetValues, ModifyTarget } from "../types/targets.types";
...
function onDiceCountMultiplier(target: AllTargetValues, mod: Extract <ModifyTarget, {type: "multiplyDiceCount"}>) :AllTargetValues {
    if (!isDiceInterface(target)) {
        devConsoleWarn(`${target} is not a DiceInterface, can't multiply the count`, target)
        return target
    }
...
function onValueMultiplier(
  ...
  mod: Extract<ModifyTarget, { type: "multiplyValue" }>,
    ...
    if (typeof target !== "number") {
        devConsoleWarn(`${target} is not a number, can't multiply its value`, target)
        return target
    }
...
export function applyTargetMods (target: AllTargetValues, mod: ModifyTarget ) : AllTargetValues {
