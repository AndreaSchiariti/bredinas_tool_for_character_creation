...
import { type AbilityProp } from "../../types/character_utils";
...
export function AbilityBox({ ability }: AbilitiesAndSkillProp): JSX.Element {
  ...
  const characterAbility = useMemo<AbilityProp>(() => {
   const retrievedAbility = character.abilities.find(pcAbility => pcAbility.name === ability )
  ...
  return (
    <div className={`ability-box-container grid-area-${ability} ${skillsWithAbilities ? "ability-box-with-skills" : ""}`}>
