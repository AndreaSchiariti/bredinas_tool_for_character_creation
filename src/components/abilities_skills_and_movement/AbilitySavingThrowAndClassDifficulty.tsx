...
import type { AbilityProp, SkillProp } from "../../types/character_utils";
...
export function AbilitySavingThrowAndClassDifficulty({characterAbility, info} : AbilityTSAndCSType) :JSX.Element {
    const character = useCharacterContext()
    const {t} = useTranslation()

    const characterSavingThrow = useMemo<SkillProp>(() => {
        const retrievedSavingThrow = character.skills.find((pcSavingThrow) => pcSavingThrow.name === `${characterAbility.name}SavingThrow`)
